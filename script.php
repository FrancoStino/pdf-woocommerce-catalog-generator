...
// Include bulk action for save and delete file. Add notice for bulk actions.
include_once 'bulk-actions.php';
...
function add_export_pdf_bulk_action($bulk_actions) {
    $bulk_actions['export_pdf'] = 'Genera catalogo in PDF';
    return $bulk_actions;
}
...
add_action('admin_notices', 'error_file_not_found');

function error_file_not_found() {
	// Verifica se è presente il parametro 'success' nell'URL per il salvataggio dei prodotti
	if (isset($_GET['file_found']) && $_GET['file_found'] === 'false') {
        echo '<div class="notice notice-error is-dismissible">
            <p>Impossibile generare il catalogo in PDF. Nessun prodotto è stato salvato.</p>
        </div>';
	}
}
...
add_action('admin_action_export_pdf', 'handle_export_pdf_bulk_action');
function handle_export_pdf_bulk_action() {
	$upload_dir = wp_upload_dir();
	$selected_products_file = $upload_dir['basedir'] . '/selected_products.txt';
	
	// Controlla se sono stati selezionati dei prodotti
    if (!file_exists($selected_products_file)) {
        wp_redirect(admin_url().'edit.php?post_type=product&file_found=false'); // Reindirizza l'utente alla pagina precedente
        exit();
    }
	...
	<div style="display:flex;flex-direction: column;align-content: center;align-items: center;">
        <div style="display: flex;width: 30vw;height: 100vh;font-family: 'Poppins', sans-serif;font-weight: bold;flex-direction: column;justify-content: space-between;">
            <div style="margin-bottom: 20px;">
                ...
                <input type="text" id="company_name" name="company_name" style="display: block; width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box;" required>
            </div>
            <div style="margin-bottom: 20px;">
                <?php
            ...
            <div style="margin-bottom: 20px; display: flex;">
                ...
                <div style="display: flex; flex-direction: column; justify-content: center; flex: 1;">
                    <input type="number" id="custom_price" name="custom_price" style="padding: 10px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box;" required value="0">
                </div>
            </div>
            <div style="margin-bottom: 20px;">
            ...
            <div style="margin-bottom: 20px;">
                <button type="submit" style="display: block; width: 100%; padding: 10px; background-color: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer;">Genera PDF</button>
            </div>
        </div>
    </div>
</form>
...
function generate_pdf_ajax() {
	...
	if (!empty($_FILES['image_upload']['tmp_name'])) {
		...
		$presentation_first_page .= '<img src="' . $temp_file_path . '" alt="Immagine" style="width: 50%;">';
